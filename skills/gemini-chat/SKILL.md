...
```
uv run ~/.config/cognition/skills/gemini-chat/scripts/chat.py
```
...
```
uv run ~/.config/cognition/skills/gemini-chat/scripts/chat.py -m "explain quantum computing simply"
```

### With a system prompt

```
uv run ~/.config/cognition/skills/gemini-chat/scripts/chat.py --system "You are a senior Rust developer"
```

### Resume a previous conversation

```
uv run ~/.config/cognition/skills/gemini-chat/scripts/chat.py --history chat.json
```

### With Google Search grounding (like AI Mode)

```
uv run ~/.config/cognition/skills/gemini-chat/scripts/chat.py --search
uv run ~/.config/cognition/skills/gemini-chat/scripts/chat.py --search -m "what happened in tech news today?"
```
...
1. Start an interactive session and relay the conversation:
   ```
   uv run ~/.config/cognition/skills/gemini-chat/scripts/chat.py --system "<context about what to discuss>"
   ```

2. For a quick question, use single-message mode:
   ```
   uv run ~/.config/cognition/skills/gemini-chat/scripts/chat.py -m "<question>" --json
   ```

3. To continue a previous conversation:
   ```
   uv run ~/.config/cognition/skills/gemini-chat/scripts/chat.py --history /tmp/gemini-chat.json
   ```
